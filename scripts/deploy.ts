...
async function main() {
  ...
  const exponentialLove = await ExponentialLove.deploy(daoAddress);
  ...
  try {
    ...
    await hre.run("verify:verify", { network: network.name, address: await exponentialLove.getAddress(), constructorArguments: [daoAddress], })
